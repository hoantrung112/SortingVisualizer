...
public class Controller {
	BubbleSort myGraph = new BubbleSort();
	...
	void randomize(ActionEvent event) {
		List<Rectangle> ele = new ArrayList<>();
		for (int i = 0; i < 41; i++) {
			ele.add((Rectangle) mainPane.getChildren().get(i));
		}
		myGraph.initData(ele);
	}

	@FXML
	void execute(ActionEvent event) {
		myGraph.execAlgorithm(mainPane);
	}
